...
import { Block, Inline, Path, childNodes, inlineChildren, toInlineArray, withChildNodes, toBlockArray, marks } from './model/node';
import { Selection, anchorNode, anchorOffset, focusNode, focusOffset, range } from './model/selection';
import { State, withRoot, withSelection } from './model/state';
import { annotateSelection, clearSelectionAnnotations, selection } from './annotation';
import { Node, findTextBlockNodeAncestor, map } from './node';
...
function mergeSimilarInlineLeaves(inlineLeaves: Array<Inline>): Array<Inline> {
  ...
  for (let i = 1; i < inlineLeaves.length; i++) {
    ...
    if (current._tag === 'Text' && next._tag === 'Text') {
      ...
      if (cMarks.length === nMarks.length && cMarks.every((m, j) => m.contents.name === nMarks[j].contents.name)) {
        current = {
          _tag: 'Text',
          text: Text.withText(current.text.contents.text + next.text.contents.text, current.text),
        };
      } else {
        result.push(current);
        current = next;
      ...
      if (c._tag === 'InlineChildren') {
        ...
        return {
          ...
          value: withChildNodes(
            inlineChildren(mergeSimilarInlineLeaves(removeExtraEmptyTextLeaves(list))),
            bn,
          ),
        };
      }
...
export function translateReducedTextBlockSelection(root: Block, state: State): State {
  ...
  const [aP, aO] = translatePath(state.contents.root, root, anchorNode(sel), anchorOffset(sel));
  const [fP, fO] = translatePath(state.contents.root, root, focusNode(sel), focusOffset(sel));
...
function translatePath(oldBlock: Block, newBlock: Block, path: Path, offset: number): [Path, number] {
  ...
  if (cOld._tag === 'InlineChildren' && cNew._tag === 'InlineChildren') {
    ...
    if (lastIndex !== undefined) {
      const pOff = parentOffset(toInlineArray(cOld.inlineChildren), lastIndex, offset);
...
function validateAllowedGroups(allowedGroups: Set<string> | null, group: string, name: string): Array<string> {
  if (!allowedGroups) return [];
  if (allowedGroups.has(group) || allowedGroups.has(name)) {
    return [];
  }
  return [`Group ${group} is not in allowed groups [${Array.from(allowedGroups).join(', ')}]`];
}

function validateAllowedMarks(allowedMarks: Set<string> | null, leaf: Inline): Array<string> {
  ...
  return [
    `Inline node is only allowed the following marks: ${Array.from(allowedMarks).join(
      ',',
...
function validateInlineLeaf(
  ...
  const definition = elementDefinitionWithDefault(InlineElement.element(leaf.inlineElement), spec);
...
function validateEditorBlockNode(spec: Spec, allowedGroups: Set<string> | null, node: Block): Array<string> {
  ...
  if (c._tag === 'BlockChildren') {
    if (contentType._tag === 'BlockNodeType') {
      const groups = contentType.allowedGroups._tag === 'Some' ? contentType.allowedGroups.value : null;
    ...
    return [`I was expecting blocknode content type, but instead I got ${contentType._tag}`];
  } else if (c._tag === 'InlineChildren') {
    if (contentType._tag === 'TextBlockNodeType') {
      const groups = contentType.allowedGroups._tag === 'Some' ? contentType.allowedGroups.value : null;
      const marks = contentType.allowedMarks._tag === 'Some' ? contentType.allowedMarks.value : null;
      const results: Array<string> = [];
    ...
    return [`I was expecting textblock content type, but instead I got ${contentType._tag}`];
  } else {
    // Leaf
    if (contentType._tag === 'BlockLeafNodeType') {
      return [];
    }
    return [`I was expecting leaf content type, but instead I got ${contentType._tag}`];
  }
}
