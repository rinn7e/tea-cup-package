...
export function elementDefinition(
  contents: Internal.ElementDefinitionContents,
): ElementDefinition {
  return {
...
export function blockNode(allowedGroups: Array<string>): ContentType {
  return {
    _tag: 'BlockNodeType',
    allowedGroups:
      allowedGroups.length === 0 ? none : some(new Set(allowedGroups)),
  }
}

export function textBlock(config: {
  ...
  return {
    ...
    allowedGroups:
      config.allowedGroups.length === 0
        ? none
        : some(new Set(config.allowedGroups)),
    allowedMarks:
      config.allowedMarks.length === 0
        ? none
        : some(new Set(config.allowedMarks)),
  }
}

export function defaultElementDefinition(
  name_: string,
  group_: string,
