...
export function findTextChanges(htmlNode: HtmlNode, domNode: DomNode): Either<string, Array<TextChange>> {
...
function findTextChangesRec(
  ...
  if (htmlNode._tag === 'ElementNode') {
    ...
    if (domNode.nodeType !== domElementNodeType) {
      return left('Dom node is a text node, but I was expecting an element node');
    }
    ...
    if (htmlTagUpper !== domTagUpper) {
      return left(`Dom node's tag was ${domNode.tagName || ''}, but I was expecting ${htmlNode.name}`);
    }
    ...
    for (let i = 0; i < htmlNode.children.length; i++) {
      ...
      const res = findTextChangesRec(htmlChild, domChild, [i, ...backwardsNodePath]);
      if (res._tag === 'Left') {
        return res;
      }
    ...
    if (domNode.nodeType !== domTextNodeType) {
      return left('Dom node was an element node, but I was expecting a text node');
    }
    if (domNode.nodeValue === null) {
      return left('Dom node is a text node, but has no value');
    }
    const domNodeSanitizedText = domNode.nodeValue === zeroWidthSpace ? '' : domNode.nodeValue;
