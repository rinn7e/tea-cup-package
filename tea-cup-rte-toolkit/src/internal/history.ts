...
export class BoundedDeque<T> {
  ...
  constructor(maxSize: number, items: Array<T> = []) {
  ...
  pushBack(item: T): BoundedDeque<T> {
    ...
    if (nextItems.length > this.maxSize) {
      return new BoundedDeque<T>(
        this.maxSize,
        nextItems.slice(nextItems.length - this.maxSize),
      )
    }
    return new BoundedDeque<T>(this.maxSize, nextItems)
  }
  ...
  last(): Option<T> {
    return this.items.length > 0
      ? some(this.items[this.items.length - 1])
      : none
  }
