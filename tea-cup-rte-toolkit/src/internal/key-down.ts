...
import { CommandMap, namedCommandListFromKeyboardEvent } from '../config/command';
...
export function shouldPreventDefault(
  ...
  const res = handleKeyDownEvent(commandMap, spec, editorInstance, keyboardEvent);
...
export function handleKeyDownEvent(
  ...
  const namedCommandList = namedCommandListFromKeyboardEvent(shortKey(editorInstance), event, commandMap);
