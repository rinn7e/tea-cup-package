...
import { Block, Inline, block, childNodes, element, inlineChildren, parent, plainText } from '../model/node';
import { Selection, anchorNode, anchorOffset, caret, focusOffset, isCollapsed } from '../model/selection';
...
import { removeRange, splitTextBlock, joinBackward, joinForward } from '../commands';
 ...
 */
export function handlePaste(event: PasteEvent, spec: Spec, editor_: Editor): Editor {
  const commandArray: Array<[string, { _tag: 'TransformCommand'; transform: Transform }]> = [
    [
      'pasteHtml',
      {
...
export function pasteText(text: string): Transform {
  return (editorState: State): Either<string, State> => {
    ...
    const tbOpt = findTextBlockNodeAncestor(anchorNode(selection), editorState.contents.root);
    if (tbOpt._tag === 'None') {
...
export function pasteInlineArray(inlineFragment: Array<Inline>): Transform {
  return (editorState: State): Either<string, State> => {
    ...
    if (inlineNode._tag === 'Text') {
      const [previous, nextText] = splitTextLeaf(anchorOffset(selection), inlineNode.text);
      const newFragment = [
      ...
      const newSelection = caret([...path, index + inlineFragment.length + 1], 0);
      return right(withSelection(some(newSelection), withRoot(replaceResult.right, editorState)));
    } else {
      const replaceResult = replaceWithFragment(
        anchorNodePath,
...
export function pasteBlockArray(blockFragment: Array<Block>): Transform {
  return (editorState: State): Either<string, State> => {
    ...
    const parentNodeOpt = nodeAt(parentPath, { _tag: 'Block', value: editorState.contents.root });
    ...
    if (cNodes._tag === 'BlockChildren') {
      ...
      const index = anchorNodePath[anchorNodePath.length - 1];
      const newSelection = caret([...parentPath, index + blockFragment.length - 1], 0);
      return right(withSelection(some(newSelection), withRoot(replaceResult.right, editorState)));
    } else {
      // InlineChildren
      ...
      const annotatedSelectionRoot = annotateSelection(splitSelection, splitEditorState.contents.root);
      ...
      const joinBeginningState = joinForwardStateRes._tag === 'Right' ? joinForwardStateRes.right : addNodesEditorState;

      const annotatedSelection = clearSelectionAnnotations(joinBeginningState.contents.root); // Wait, selectionFromAnnotations in Elm
      // Let's just find the selection path: we can clear annotations or lookup
      const joinEndStateRes = joinBackward(withSelection(some(splitSelection), joinBeginningState));
      const joinEndState = joinEndStateRes._tag === 'Right' ? joinEndStateRes.right : joinBeginningState;

      return right(withRoot(clearSelectionAnnotations(joinEndState.contents.root), joinEndState));
    }
  };
}
