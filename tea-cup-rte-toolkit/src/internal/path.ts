...
function domToEditorInlineLeafTree(spec: Spec, tree: InlineTree, path: Path): Option<Path> {
  if (tree._tag === 'LeafNode') {
    ...
    const structure = MarkDefinition.toHtmlNode(markDefinition)(n.mark, childNodesPlaceholder);
...
export function domToEditor(spec: Spec, node: Block, path: Path): Option<Path> {
  ...
  const structure = ElementDefinition.toHtmlNode(elementDefinition)(parameters, childNodesPlaceholder);
...
export function editorToDom(spec: Spec, node: Block, path: Path): Option<Path> {
  ...
  const childPathOpt = pathToChildContentsFromElementParameters(spec, element(node));
...
function pathToChildContentsFromMark(spec: Spec, mark: Mark): Option<Path> {
  ...
  const markStructure = MarkDefinition.toHtmlNode(markDefinition)(mark, childNodesPlaceholder);
  return pathToChildContents(markStructure);
}

function pathToChildContentsFromElementParameters(spec: Spec, parameters: Element): Option<Path> {
  const elementDefinition = elementDefinitionWithDefault(parameters, spec);
  const nodeStructure = ElementDefinition.toHtmlNode(elementDefinition)(parameters, childNodesPlaceholder);
...
function pathToChildContentsFromInlineTreePath(
  ...
  if (tree._tag === 'LeafNode') {
    ...
    const restOpt = pathToChildContentsFromInlineTreePath(spec, array, n.children, xs);
