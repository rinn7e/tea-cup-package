...
import { EditorChange, InitEvent, InputEvent, KeyboardEvent, PasteEvent } from './event';
import { History, contents, empty, fromContents, BoundedDeque } from './history';
 ...
 */
export type Editor = { readonly _tag: 'Editor'; readonly contents: EditorContents };
...
export function forceCompleteRerender(e: Editor): Editor {
  return {
    _tag: 'Editor',
    contents: { ...e.contents, completeRerenderCount: e.contents.completeRerenderCount + 1 },
  };
}

export function areSelectionsEqual(a: Option<Selection>, b: Option<Selection>): boolean {
  if (a._tag === 'Some' && b._tag === 'Some') {
    ...
    if (sa.contents.anchorNode.join(':') !== sb.contents.anchorNode.join(':')) return false;
    if (sa.contents.focusNode.join(':') !== sb.contents.focusNode.join(':')) return false;
    return true;
  }
...
function areMarksEqual(a: Array<Mark>, b: Array<Mark>): boolean {
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    if (a[i].contents.attributes.length !== b[i].contents.attributes.length) return false;
  ...
  if (cnA._tag === 'BlockChildren' && cnB._tag === 'BlockChildren') {
    ...
    for (let i = 0; i < arrA.length; i++) {
      ...
      if (leafA._tag === 'Text' && leafB._tag === 'Text') {
        ...
        if (!areMarksEqual(leafA.text.contents.marks, leafB.text.contents.marks)) return false;
...
export function areStatesEqual(a: State, b: State): boolean {
  return (
    areSelectionsEqual(a.contents.selection, b.contents.selection) && areBlocksEqual(a.contents.root, b.contents.root)
  );
}
...
export function handleUndo(editor_: Editor): Editor {
  ...
  const [maybeState, newUndoDeque] = findNextState(editorState, editorHistory.undoDeque);
  ...
  return incrementChangeCount(withHistory(fromContents(newHistory), withState(newState, editor_)));
}
...
export function handleRedo(editor_: Editor): Either<string, Editor> {
  ...
  return right(incrementChangeCount(withHistory(fromContents(newHistory), withState(newState, editor_))));
}

export function updateEditorState(action: string, newState: State, editor_: Editor): Editor {
...
export function updateEditorStateWithTimestamp(
  ...
  if (firstOpt._tag === 'None') {
    ...
    if (
      lastAction === action &&
      timestamp !== 0 &&
      timestamp - editorHistory.lastTextChangeTimestamp < editorHistory.groupDelayMilliseconds
    ) {
...
export function applyInternalCommand(action: 'Undo' | 'Redo', editor_: Editor): Either<string, Editor> {
  if (action === 'Undo') {
...
export function applyCommand(cmd: NamedCommand, spec: Spec, editor_: Editor): Either<string, Editor> {
  ...
  if (command._tag === 'InternalCommand') {
    ...
    return right(forceReselection(updateEditorState(name, reducedState, editor_)));
  }
}
