...
import { Children, blockChildren, inlineChildren } from '../model/node'
...
function resultFilterMap<A, B>(
  f: (x: A) => Either<string, B>,
  xs: Array<A>,
): [Array<B>, Array<string>] {
...
export function htmlToElementArray(
  spec: Spec,
  html: string,
): Either<string, Array<Fragment>> {
  const htmlNodeArray = stringToHtmlNodeArray(html)
  const [newArray, errList] = resultFilterMap(
    (node) => htmlNodeToEditorFragment(spec, [], node),
    htmlNodeArray,
  )
...
export function htmlNodeToEditorFragment(
      ...
      inlineFragment: [
        {
          ...
          text: Text.withMarks(
            marks,
            Text.withText(
              node.text.replace(new RegExp(zeroWidthSpace, 'g'), ''),
              Text.empty,
            ),
          ),
        },
  ...
  if (maybeElementAndChildren !== null) {
    ...
    if (contentType._tag === 'InlineLeafNodeType') {
      ...
      const childArr = children.map((child) =>
        htmlNodeToEditorFragment(spec, [], child),
      )
  ...
  const newChildren = children.map((child) =>
    htmlNodeToEditorFragment(spec, newMarks, child),
  )
  return arrayToFragment(newChildren)
}

export function htmlNodeToMark(
  spec: Spec,
  node: HtmlNode,
): Option<[Mark, Array<HtmlNode>]> {
...
export function reduceEditorFragmentArray(
  fragmentArray: Array<Fragment>,
): Array<Fragment> {
...
export function arrayToChildNodes(
  ...
  if (fragment._tag === 'InlineFragment') {
    if (contentType._tag === 'TextBlockNodeType') {
      ...
      return left(
        "I received an inline leaf fragment, but the node I parsed doesn't accept this child type",
      )
    }
  } else {
    // BlockFragment
    if (contentType._tag === 'BlockNodeType') {
      ...
      return left(
        "I received a block node fragment, but the node I parsed doesn't accept this child type",
      )
    }
  }
}
...
export function markDefinitionWithDefault(
  markVal: Mark,
  spec: Spec,
