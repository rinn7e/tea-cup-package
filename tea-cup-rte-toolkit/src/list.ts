...
import { Block, Path, block, blockChildren, childNodes, toBlockArray, withChildNodes } from './model/node';
import { Element, element } from './model/element';
import { orderedList, unorderedList, listItem } from './definitions';
import { Transform, CommandMap, emptyCommandMap, inputEvent, key, set, transform } from './config/command';
import { enter, backspace, delete as deleteKey, alt, return as returnKey } from './config/keys';
import { Selection, anchorNode, anchorOffset, focusOffset, caret, isCollapsed, normalize } from './model/selection';
import { State, withRoot, withSelection } from './model/state';
import { Node, findAncestor, isEmptyTextBlock, nodeAt, replace, replaceWithFragment } from './node';
import { annotateSelection, clear, clearSelectionAnnotations, doLift, selectionFromAnnotations } from './annotation';
import { wrap as wrapBlock, splitBlock } from './commands';
...
export type ListDefinition = { readonly _tag: 'ListDefinition'; readonly contents: ListDefinitionContents };

export function listDefinition(contents: ListDefinitionContents): ListDefinition {
...
export function findListItemAncestor(itemParams: Element, path: Path, root: Block): Option<[Path, Block]> {
  const res = findAncestor(
    (p, node) => node._tag === 'Block' && node.value.contents.parameters.contents.name === itemParams.contents.name,
    path,
    root,
  );
  if (res._tag === 'Some') {
...
export function split(def: ListDefinition): Transform {
  return splitBlock((path: Path, root: Block) => findListItemAncestor(item(def), path, root));
}
...
export function isListNode(def: ListDefinition, node: Node): boolean {
  ...
  return name === ordered(def).contents.name || name === unordered(def).contents.name;
}
...
export function defaultCommandMap(def: ListDefinition): CommandMap {
  return set(
    ...
    [
      ['liftEmptyListItem', { _tag: 'TransformCommand', transform: liftEmpty(def) }],
...
export function lift(def: ListDefinition): Transform {
  return (editorState: State): Either<string, State> => {
    ...
    const startOpt = findListItemAncestor(item(def), anchorNode(norm), editorState.contents.root);
    ...
    const newSelection = selectionFromAnnotations(liftedRoot, anchorOffset(norm), focusOffset(norm));

    return right(
      withSelection(
        newSelection,
        withRoot(clear('lift', clearSelectionAnnotations(liftedRoot)), editorState),
      ),
    );
  };
}
...
export function liftEmpty(def: ListDefinition): Transform {
  return (editorState: State): Either<string, State> => {
    ...
    if (!isCollapsed(sel) || anchorOffset(sel) !== 0) {
      return left('I can only lift collapsed selections at the beginning of a text node');
    }

    const itemOpt = findListItemAncestor(item(def), anchorNode(sel), editorState.contents.root);
