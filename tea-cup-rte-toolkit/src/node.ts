...
import { Selection, anchorNode, anchorOffset } from './model/selection'
...
export function indexedMap(
  func: (path: Path, node: Node) => Node,
  node: Node,
): Node {
  return indexedMapRec([], func, node)
}

function indexedMapRec(
  path: Path,
  func: (path: Path, node: Node) => Node,
  node: Node,
): Node {
  ...
  if (applied._tag === 'Block') {
    ...
    if (c._tag === 'BlockChildren') {
      const mapped = toBlockArray(c.blockChildren.array).map((v, i) => {
        const res = indexedMapRec([...path, i], func, {
          _tag: 'Block',
          value: v,
        })
        return res._tag === 'Block' ? res.value : v
      ...
      const mapped = toInlineArray(c.inlineChildren.contents.array).map(
        (v, i) => {
          const res = indexedMapRec([...path, i], func, {
            _tag: 'Inline',
            value: v,
...
export function foldl<B>(
  func: (node: Node, acc: B) => B,
  acc: B,
  node: Node,
): B {
  if (node._tag === 'Block') {
    ...
    if (c._tag === 'InlineChildren') {
      children = toInlineArray(c.inlineChildren.contents.array).map((v) => ({
        _tag: 'Inline' as const,
        value: v,
      }))
    } else if (c._tag === 'BlockChildren') {
      children = toBlockArray(c.blockChildren.array).map((v) => ({
        _tag: 'Block' as const,
        value: v,
      }))
    }
...
export function indexedFoldl<B>(
...
export function indexedFoldr<B>(
  func: (path: Path, node: Node, acc: B) => B,
  acc: B,
  node: Node,
): B {
...
export function removeInRange(start: Path, end: Path, root: Block): Block {
  ...
  const res = indexedMap(
    (path, node) => {
      const pathStr = path.join(':')
      if (
        comparePaths(path, start) === 'GT' &&
        comparePaths(path, end) === 'LT'
      ) {
        // Return empty node
        if (node._tag === 'Block') {
          return {
            _tag: 'Block',
            value: withChildNodes(blockChildren([]), node.value),
          }
        } else {
          const il = node.value
          if (il._tag === 'Text') {
            return {
              _tag: 'Inline',
              value: { _tag: 'Text', text: Text.withText('', il.text) },
            }
          }
        }
      }
...
export function isEmptyTextBlock(node: Block): boolean {
  ...
  if (c._tag === 'InlineChildren') {
    ...
    if (
      arr.length === 1 &&
      arr[0]._tag === 'Text' &&
      arr[0].text.contents.text.length === 0
    ) {
      return true
    }
  }
...
export function selectionIsBeginningOfTextBlock(
  sel: Selection,
  root: Block,
): boolean {
...
export function selectionIsEndOfTextBlock(
  sel: Selection,
  root: Block,
): boolean {
  const path = anchorNode(sel)
...
export function findTextBlockNodeAncestor(
  path: Path,
  root: Block,
): Option<[Path, Block]> {
...
export function splitBlockAtPathAndOffset(
  path: Path,
  offset: number,
  node: Block,
): Option<[Block, Block]> {
  ...
  if (c._tag === 'BlockChildren') {
    ...
    if (n._tag === 'Text') {
      ...
      const beforeArr = [
        ...arr.slice(0, x),
        { _tag: 'Text' as const, text: before },
      ]
      const afterArr = [
        { _tag: 'Text' as const, text: after },
        ...arr.slice(x + 1),
      ]
      return some([
...
export function toggleMark(
  ...
  return replace(
    path,
    { _tag: 'Inline', value: { _tag: 'Text', text: newText } },
    root,
  )
}
