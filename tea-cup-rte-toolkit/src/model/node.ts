 ...
 */
export type BlockChildren = { readonly _tag: 'BlockArray'; readonly array: Array<Block> };
 ...
 */
export type InlineChildren = { readonly _tag: 'InlineLeafArray'; readonly contents: InlineLeafArrayContents };
 ...
 */
export function toInlineArray(arr: InlineChildren | Array<Inline>): Array<Inline> {
  if (Array.isArray(arr)) {
    return arr;
  }
...
export type InlineTree =
  | { readonly _tag: 'MarkNode'; readonly contents: { readonly mark: Mark; readonly children: Array<InlineTree> } }
 ...
 */
export type Inline =
  | { readonly _tag: 'InlineElement'; readonly inlineElement: InlineElementMod.InlineElement }
...
function groupWhile<A>(pred: (x: A, y: A) => boolean, list: Array<A>): Array<[A, Array<A>]> {
 ...
 */
export function marksToMarkNodeList(markLists: Array<Array<Mark>>): Array<InlineTree> {
...
export function inlineTreeToPaths(forwardsPath: Path, tree: Array<InlineTree>): Array<Path> {
  const paths: Array<Path> = [];
  tree.forEach((n, i) => {
    if (n._tag === 'LeafNode') {
      paths.push([...forwardsPath, i]);
    } else {
      const childPaths = inlineTreeToPaths([...forwardsPath, i], n.contents.children);
