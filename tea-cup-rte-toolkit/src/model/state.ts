...
export type State = {
  readonly _tag: 'State'
  readonly contents: StateContents
}
  ...
  if (x._tag === 'Text') {
    if (y._tag === 'Text') {
      ...
      if (xText === '' && !Text.annotations(x.text).has(selectionAnnot)) {
        ...
        yText === '' &&
        !Text.annotations(y.text).has(selectionAnnot)
      ) {
  ...
  if (x._tag === 'Text') {
    if (y._tag === 'Text') {
      ...
      if (areMarksEqual(marksX, marksY)) {
        const mergedText = Text.withText(
          Text.text(x.text) + Text.text(y.text),
          x.text,
        )
        return mergeSimilarInlineLeaves([
          { _tag: 'Text', text: mergedText },
          ...xs,
        ])
      } else {
    ...
    (x) => {
      if (x._tag === 'Block') {
        ...
        if (cn._tag === 'InlineChildren') {
          ...
          const reducedList = mergeSimilarInlineLeaves(
            removeExtraEmptyTextLeaves(arr),
          )
          return {
            _tag: 'Block',
  ...
  if (oldCn._tag === 'InlineChildren') {
    ...
    if (newCn._tag === 'InlineChildren') {
      const pOff = parentOffset(
        toInlineArray(oldCn.inlineChildren),
        lastIndex,
        offset,
      )
...
export function validate(
  ...
  const errors = validateEditorBlockNode(
    spec,
    some(new Set(['root'])),
    rootBlock,
  )
...
function validateAllowedGroups(
  ...
  return [
    `Group ${group} is not in allowed groups [${Array.from(groups).join(', ')}]`,
  ]
}
...
function validateEditorBlockNode(
  ...
  const allowedGroupsErrors = validateAllowedGroups(
    allowedGroups,
    definition.contents.group,
    definition.contents.name,
  )
  ...
  if (cn._tag === 'BlockChildren') {
    ...
    if (contentType._tag === 'TextBlockNodeType') {
      ...
      for (const leaf of arr) {
        errors.push(
          ...validateInlineLeaf(
            spec,
            contentType.allowedGroups,
            contentType.allowedMarks,
            leaf,
          ),
        )
      }
    ...
    if (contentType._tag === 'BlockLeafNodeType') {
      ...
      return [
        `I was expecting leaf blockleaf content type, but instead I got ${toStringContentType(contentType)}`,
      ]
    }
  }
}
