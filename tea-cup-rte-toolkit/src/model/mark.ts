...
export type MarkOrder = { readonly _tag: 'MarkOrder'; readonly contents: Record<string, number> };
...
export function toggle(toggleAction: ToggleAction, order: MarkOrder, mark_: Mark, marks: Array<Mark>): Array<Mark> {
  const isMember = marks.some(m => name(m) === name(mark_));
  if (toggleAction._tag === 'Remove' || (toggleAction._tag === 'Flip' && isMember)) {
