...
import { selection as constSelection, selectable as constSelectable, lift as constLift } from './internal/constants';
import { Block, Inline, Path, block, blockChildren, childNodes, element, toBlockArray, withChildNodes, withElement } from './model/node';
import { Element, annotations as getElementAnnotations, withAnnotations as withElementAnnotations } from './model/element';
 ...
 */
export function addAtPath(annotation: string, path: Path, node: Block): Either<string, Block> {
 ...
 */
export function removeAtPath(annotation: string, path: Path, node: Block): Either<string, Block> {
  const nodeOpt = nodeAt(path, { _tag: 'Block', value: node });
  if (nodeOpt._tag === 'None') {
    return left('No block found at path');
  }
...
function toggle(
  ...
  if (node._tag === 'Block') {
    ...
    if (il._tag === 'InlineElement') {
      const newParams = toggleElementParameters(func, annotation, il.inlineElement.contents.element);
      return {
        _tag: 'Inline',
        value: {
          _tag: 'InlineElement',
      ...
      return {
        ...
        value: {
          _tag: 'Text',
          text: Text.withAnnotations(func(annotation, Text.annotations(il.text)), il.text),
        },
      };
    }
  }
...
export function annotateSelection(selection_: Selection, node: Block): Block {
  return addSelectionAnnotationAtPath(focusNode(selection_), addSelectionAnnotationAtPath(anchorNode(selection_), node));
}
 ...
 */
export function selectionFromAnnotations(node: Block, anchorOffset: number, focusOffset: number): Option<Selection> {
...
function findNodeRangeFromSelectionAnnotations(node: Block): Option<[Path, Path]> {
...
export function isSelectable(node: Node): boolean {
  if (node._tag === 'Block') {
    ...
    if (ln._tag === 'Text') {
      ...
      return getElementAnnotations(ln.inlineElement.contents.element).has(selectable);
    }
  }
}

function groupWhile<A>(pred: (a: A, b: A) => boolean, list: Array<A>): Array<[A, Array<A>]> {
...
function liftConcatMapFunc(node: Node): Array<Node> {
  if (node._tag === 'Block') {
    ...
    const groups = groupWhile(
      (n1, n2) => getElementAnnotations(element(n1)).has(lift) === getElementAnnotations(element(n2)).has(lift),
      childrenArray,
    );
