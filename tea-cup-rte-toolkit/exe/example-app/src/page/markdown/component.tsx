...
import type { Model, Msg, EditorType } from './type'
  ...
  return (
    <div>
      ...
      <p className="page-description">
        This example demonstrates switching between a plain Markdown source editor and a WYSIWYG editor. It converts the editor state to Markdown using <code>turndown</code> and parses Markdown back to the editor state using <code>marked</code>.
      </p>

      {/* Editor Selector Controls */}
      <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.5rem', alignItems: 'center' }}>
        <label className="radio-label" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontWeight: 600 }}>
          <input
            type="radio"
            name="editorType"
            checked={isWysiwyg}
            onChange={() => dispatch({ _tag: 'EditorChange', editorType: 'WYSIWYG' })}
        ...
        <label className="radio-label" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontWeight: 600 }}>
          <input
            ...
            name="editorType"
            checked={!isWysiwyg}
            onChange={() => dispatch({ _tag: 'EditorChange', editorType: 'Markdown' })}
            style={{ width: '18px', height: '18px', accentColor: 'var(--accent)' }}
          />
          Raw Markdown Text
        </label>
      ...
      {/* Error Alert */}
      {model.markdownError && (
        <div style={{ backgroundColor: 'rgba(239, 68, 68, 0.15)', border: '1px solid var(--danger)', borderRadius: '8px', padding: '1rem', marginBottom: '1.5rem', color: 'var(--danger)', fontSize: '0.95rem' }}>
        ...
        <div className="markdown-split-layout">
          <div className="markdown-panel">
            ...
            <textarea
              ...
              onChange={(e) => dispatch({ _tag: 'TextAreaChange', text: e.target.value })}
