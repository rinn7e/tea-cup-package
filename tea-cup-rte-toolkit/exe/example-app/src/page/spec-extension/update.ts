...
import type { Model, Msg, InsertCaptionedImageModal } from './type'
...
export const captionedImage = elementDefinition({
  ...
  toHtmlNode: (params: RteElement, children: Array<HtmlNode>): HtmlNode => {
    ...
    const captionOpt = findStringAttribute('caption', params.contents.attributes)
      ...
      if (img && img._tag === 'ElementNode' && img.name === 'img') {
        ...
        for (const [k, v] of img.attributes) {
          if (k === 'src') {
            ...
            attributes.push({ _tag: 'StringAttribute', key: 'title', value: v })
          } else if (k === 'data-caption') {
            attributes.push({ _tag: 'StringAttribute', key: 'caption', value: v })
          }
        }
        if (hasSrc) {
...
export const customSpec: Spec = pipe(
  ...
  (s) => withMarkDefinitions([...markDefinitions(s), strikethrough, underline], s)
)
  ...
  block(
    element(captionedImage, [
      { _tag: 'StringAttribute', key: 'caption', value: 'The Tea-Cup logo!' },
      {
        ...
        key: 'src',
        value: 'https://raw.githubusercontent.com/elm-explorations/rte-toolkit/master/logo.png',
      },
    ]),
    Leaf
  )
...
const docInitNode = (): Block =>
  block(
    element(doc, []),
    blockChildren([loremParagraph(), initialCaptionedImage(), loremParagraph()])
  )
...
export const update = (msg: Msg, model: Model): [Model, Cmd<Msg>] => {
  switch (msg._tag) {
    case 'EditorMsg': {
      ...
      if (msg.subMsg._tag === 'InternalMsg' && msg.subMsg.msg._tag === 'ChangeEvent') {
