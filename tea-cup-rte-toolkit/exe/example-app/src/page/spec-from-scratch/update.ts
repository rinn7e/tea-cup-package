    ...
    return {
      ...
      children: [
          ...
          children: [
            {
              ...
              name: 'div',
              attributes: [
                ['class', checked ? 'checklist-checkbox checked' : 'checklist-checkbox'],
              ],
              children: [],
            },
          ],
        },
        {
          _tag: 'ElementNode',
          name: 'div',
          attributes: [['class', checked ? 'checklist-content completed' : 'checklist-content']],
          children,
        },
      ],
    }
  },
  fromHtmlNode: (def: any, node: HtmlNode): Option<[RteElement, Array<HtmlNode>]> => {
    if (node._tag === 'ElementNode' && node.name === 'li') {
      ...
      if (wrapper && wrapper._tag === 'ElementNode' && wrapper.name === 'div') {
        ...
        if (checkDiv && checkDiv._tag === 'ElementNode' && checkDiv.name === 'div') {
          let checked = false
...
const toggleCheckboxDecoration = (
  ...
  const checkedOpt = findBoolAttribute('checked', elementVal.contents.attributes)
  const checked = checkedOpt._tag === 'Some' ? checkedOpt.value : false
...
const docInitNode = (): Block =>
  block(
    element(todoList, []),
    blockChildren([
      initialTodoNode('Port Elm Rich Text Editor Toolkit to TypeScript/React 💞', true),
      initialTodoNode('Set up monorepo workspaces and link dependencies ⚙️', true),
      initialTodoNode('Implement path-based (History API) URL routing 🔗', true),
      initialTodoNode('Port the five interactive demo pages faithfully 🌸', false),
      initialTodoNode('Verify all TS builds compile with 0 compilation errors! ✨', false),
    ])
  )
  ...
  switch (msg._tag) {
    ...
    case 'ToggleCheckedTodoItem': {
      ...
      const res = applyCommand(
        [
          'updateTodoListItem',
          transform(updateTodoListItem(path, checked)),
        ],
        todoSpec,
        model.editor.editor
      )
