  ...
  switch (msg._tag) {
    ...
    case 'ShowInsertLinkModal': {
      ...
      const hasLink = anyRange(
        (n) => {
          if (n._tag === 'Inline') {
            ...
            return getMarks(inlineNode).some(
              (m: Mark) => m.contents.name === 'link',
            )
          }
          return false
        },
      ...
      if (hasLink) {
        ...
        const res = applyCommand(
          ['removeLink', transform(toggleMark(markOrder, linkMark, Remove))],
          spec,
          model.editor,
        )
        const newEditor = res._tag === 'Right' ? res.right : model.editor
    ...
    case 'WrapInList': {
      const res = applyCommand(
        ['wrapList', transform(wrapList(defaultListDefinition, msg.listType))],
        spec,
        model.editor,
      )
      const newEditor = res._tag === 'Right' ? res.right : model.editor
...
function nextPathFunc(path: Path, root: Block): O.Option<Path> {
  ...
  if (node._tag === 'Block') {
    ...
    if (
      c._tag === 'InlineChildren' &&
      c.inlineChildren.contents.array.length > 0
    ) {
      return O.some([...path, 0])
    }
  }
